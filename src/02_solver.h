...
struct DecisionRules {
  ...
  rowvec gamma_b;        // 1x3: barb^s_t = Gamma_b * v_t  (BW eq 2.15)
  rowvec gamma_tau_h;    // 1x3: hattau^h_t = Gamma_tau^h * v_t  (BW eq 2.17)
  rowvec Gamma_k_0;      // 1x3: current-state part of hattau^k  (BW eq 3.8)
  rowvec Gamma_k_1;      // 1x3: lagged-state part of hattau^k
  rowvec gamma_theta_e;  // 1x3: theta^e_t = E_t[tau^k_{t+1}] = (Gamma_k_0*hx + Gamma_k_1)*v_t
};
...
// =====================================================================
static mat solve_dare(const Params& p, int max_iter = 5000, double tol = 1e-14)
{
  const mat& A = p.A_con;
  const mat& B = p.B_con;
...
inline DecisionRules compute_decision_rules(const Params& p)
{
  ...
  //  Log-linearising (1.21):  uc*k_{t+1} = tildebeta Et[W_{t+1} - b^s_t * uc_{t+1}]
  //  yields:  barb^s_t = (s_c/s_b) Gamma_w*hx*v_t
  ...
  //  where A = -sigma^{-1} gx^{(0)} + psi gx^{(1)}
  {
    ...
    rules.gamma_b = c1 * rules.gamma_w * rules.hx
                  - c2 * rules.hx.row(0)
                  - c2 * A_row
                  - A_row * rules.hx;
  }
  ...
  //  hattau^h_t = (1-alpha)hatz + alpha hatk - (sigma^{-1} - phi^{-1}psi)hatc - (alpha+vee-psi)hath
  //         = [alpha, 1-alpha, 0] v + coefficients x gx v
  {
    const double phi_inv_psi = (p.phi_bw > 1e-14)
                             ? p.psi_bw / p.phi_bw : 0.0;
    // BW eq (2.17): hattau^h = hatz + alphak̃̂ - (sigma⁻¹ - phi⁻¹psi)hatc - (vee - psi)hath
    // where k̃̂ = hatk - hatz - hath (eq 2.12, capital per effective labor)
    // Expanding: alphahatk + (1-alpha)hatz - (sigma⁻¹ - phi⁻¹psi)hatc - (alpha + vee - psi)hath
    rowvec e_kz = { p.alpha, 1.0 - p.alpha, 0.0 };
    rules.gamma_tau_h = e_kz
                      - (p.sigma_inv - phi_inv_psi) * rules.gx.row(0)
  ...
  //
  //  From:  b_tau hattau^k = s_b barb_{t-1} - s_c tildeW_t - b_c hatc + b_h hath
  ...
  {
    const double inv_bt = (std::abs(p.btau_bw) > 1e-14)
                        ? 1.0 / p.btau_bw : 0.0;
    ...
    rules.Gamma_k_0 = inv_bt * (
        -p.sc * rules.gamma_w
        - p.bc_bw * rules.gx.row(0)
        + p.bh_bw * rules.gx.row(1)
        + kterm
        + zterm
    );
...
// =====================================================================
inline mat simulate(const Params& p, const DecisionRules& rules,
  ...
  for (int t = 0; t < T_total; ++t) {
    if (t >= T_burn) {
      ...
      out(row, 1) = rules.gamma_theta_e(0) * k_hat
                  + rules.gamma_theta_e(1) * z
                  + rules.gamma_theta_e(2) * g_hat;

      // col 2: tau^k - BW eq (3.8) = Gamma^k_tau(0) v_t + Gamma^k_tau(1) v_{t-1}
      out(row, 2) = rules.Gamma_k_0(0) * k_hat
                  + rules.Gamma_k_0(1) * z
                  + rules.Gamma_k_0(2) * g_hat
                  + rules.Gamma_k_1(0) * k_hat_p
                  + rules.Gamma_k_1(1) * z_p
                  + rules.Gamma_k_1(2) * g_hat_p;

      // col 3: theta^{nc} - non-contingent debt (barb_{t-1} = 0)
      //  Same as tau^k but drop Gamma^k_tau(1) v_{t-1} term
      out(row, 3) = rules.Gamma_k_0(0) * k_hat
                  + rules.Gamma_k_0(1) * z
                  + rules.Gamma_k_0(2) * g_hat;
    ...
    const double k_hat_next = rules.hx(0,0) * k_hat
                            + rules.hx(0,1) * z
                            + rules.hx(0,2) * g_hat;
    const double z_next     = p.rho_z * z     + (flip(rng) ? delta_z : -delta_z);
    const double g_hat_next = p.rho_g * g_hat + (flip(rng) ? delta_g : -delta_g);
...
static double safe_cor(const vec& a, const vec& b)
{
  ...
  if (sa < 1e-15 || sb < 1e-15) return datum::nan;
