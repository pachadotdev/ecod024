...
cat("Actual forecast periods:", n_fcst_actual, "\n")
cat("Final plot range:", date_sample[plot_start], "to", date_sample[plot_end], "\n")
```
...
ggplot(df_fig7, aes(x = date)) +
  ...
  geom_line(aes(y = bhank_med), color = col_red, linewidth = 1, linetype = "dashed") +
...
## 9. Figure 8: Historical Scenario (Post-COVID Inflation)

Counterfactual analysis of the post-COVID inflationary episode. Since our 
sample ends in 2019Q4, we simulate a forward-looking scenario starting from
...
# Variable indices: [9] Inflation, [2] Output, [10] Interest Rate
pi_fcst <- fcst_scenario[, 9]
y_fcst <- fcst_scenario[, 2]
i_fcst <- fcst_scenario[, 10]

cat("Baseline forecast from", date_sample[scenario_date], "\n")
cat("  Inflation (h=0):", round(pi_fcst[1], 2), "\n")
cat("  Output (h=0):", round(y_fcst[1], 2), "\n")
cat("  Rate (h=0):", round(i_fcst[1], 2), "\n")

# Compute counterfactual scenario
n_draws_fig8 <- min(100, n_draws)

pi_cnfctl_scen <- matrix(NA, T_hor, n_draws_fig8)
y_cnfctl_scen <- matrix(NA, T_hor, n_draws_fig8)
i_cnfctl_scen <- matrix(NA, T_hor, n_draws_fig8)

for (d in seq_len(n_draws_fig8)) {
  Pi_m <- irfs_all$Pi_m[[d]]
  Y_m <- irfs_all$Y_m[[d]]
  I_m <- irfs_all$I_m[[d]]
  
  result <- compute_optpol(
    pi_fcst, y_fcst, i_fcst,
    Pi_m, Y_m, I_m,
    weights
  )
  
  pi_cnfctl_scen[, d] <- result$pi_z_optpol
  y_cnfctl_scen[, d] <- result$y_z_optpol
  i_cnfctl_scen[, d] <- result$i_z_optpol
}
```
...
# Summary statistics
pi_scen_med <- apply(pi_cnfctl_scen, 1, median)
pi_scen_lb <- apply(pi_cnfctl_scen, 1, quantile, 0.16)
pi_scen_ub <- apply(pi_cnfctl_scen, 1, quantile, 0.84)

y_scen_med <- apply(y_cnfctl_scen, 1, median)
y_scen_lb <- apply(y_cnfctl_scen, 1, quantile, 0.16)
y_scen_ub <- apply(y_cnfctl_scen, 1, quantile, 0.84)

i_scen_med <- apply(i_cnfctl_scen, 1, median)
i_scen_lb <- apply(i_cnfctl_scen, 1, quantile, 0.16)
i_scen_ub <- apply(i_cnfctl_scen, 1, quantile, 0.84)

# Build plot data
df_fig8 <- data.frame(
  horizon = rep(0:(T_hor - 1), 6),
  value = c(pi_fcst, pi_scen_med, y_fcst, y_scen_med, i_fcst, i_scen_med),
  lower = c(rep(NA, T_hor), pi_scen_lb, rep(NA, T_hor), y_scen_lb, 
            rep(NA, T_hor), i_scen_lb),
  upper = c(rep(NA, T_hor), pi_scen_ub, rep(NA, T_hor), y_scen_ub,
            rep(NA, T_hor), i_scen_ub),
  type = rep(c("Baseline Forecast", "Counterfactual"), each = T_hor, times = 3),
  variable = rep(c("Inflation", "Output Gap", "Interest Rate"), each = 2 * T_hor)
)

df_fig8$variable <- factor(df_fig8$variable, 
                            levels = c("Inflation", "Output Gap", "Interest Rate"))

ggplot(df_fig8, aes(x = horizon)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "coral", alpha = 0.5) +
  geom_line(aes(y = value, color = type, linetype = type), linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  facet_wrap(~variable, scales = "free_y", ncol = 1) +
  scale_color_manual(values = c("Baseline Forecast" = "black", 
                                  "Counterfactual" = "coral")) +
  scale_linetype_manual(values = c("Baseline Forecast" = "dashed", 
                                    "Counterfactual" = "solid")) +
  labs(
    title = "Figure 8: Counterfactual Scenario Analysis",
    subtitle = paste0("Forecast from ", date_sample[scenario_date], 
                       ". Dashed = Baseline, Solid = Optimal policy (median with 68% bands)"),
    x = "Quarters ahead",
    y = "Percent",
    color = "", linetype = ""
  ) +
