...
# Use the last date in our sample as the forecast origin
scenario_date <- nrow(vardata_dt)
fcst_lag <- 4   # Quarters of history to show before forecast
fcst_hor_fig8 <- min(20, T_hor)  # Forecast horizon for plotting

# Generate baseline forecast from final date
fcst_scenario <- var_fcst(
  vardata_dt, n_lags, constant,
  bvar_result$B_OLS, scenario_date, T_hor,
...
# Historical values (for prepending to plots)
pi_history_fig8 <- vardata_dt[, 9]
y_history_fig8 <- vardata_dt[, 2]
i_history_fig8 <- vardata_dt[, 10]

# Date range for plotting
plot_start_fig8 <- scenario_date - fcst_lag
plot_dates_fig8 <- c(date_sample[plot_start_fig8:scenario_date], 
                      date_sample[scenario_date] + (1:fcst_hor_fig8) * 0.25)
```
...
col_grey <- "#969696"
col_beige <- "#C4AE78"
col_lbeige <- scales::alpha(col_beige, 0.3)
col_red <- "#CC0000"
col_lred <- scales::alpha(col_red, 0.3)
col_blue <- "#66B2FF"
col_lblue <- scales::alpha(col_blue, 0.3)
col_orange <- "#FF9933"
col_lorange <- scales::alpha(col_orange, 0.3)

# Helper to build plot series with history prepended
# hist_vals: history values to prepend
# fcst_vals: forecast/counterfactual values (first fcst_hor_fig8 used)
build_series <- function(hist_vals, fcst_vals, fcst_hor) {
  c(hist_vals, fcst_vals[1:fcst_hor])
}

# Historical data to prepend (fcst_lag + 1 periods ending at scenario_date)
pi_hist <- pi_history_fig8[(scenario_date - fcst_lag):scenario_date]
y_hist <- y_history_fig8[(scenario_date - fcst_lag):scenario_date]
i_hist <- i_history_fig8[(scenario_date - fcst_lag):scenario_date]

# Baseline VAR forecast (prepend history, then forecast)
pi_base_plot <- build_series(pi_hist, pi_base_x, fcst_hor_fig8)
y_base_plot <- build_series(y_hist, y_base_x, fcst_hor_fig8)
i_base_plot <- build_series(i_hist, i_base_x, fcst_hor_fig8)

# History line (history part, then NaN for forecast period)
pi_hist_plot <- c(pi_hist, rep(NA, fcst_hor_fig8))
y_hist_plot <- c(y_hist, rep(NA, fcst_hor_fig8))
i_hist_plot <- c(i_hist, rep(NA, fcst_hor_fig8))

# Build counterfactual series (prepend history to counterfactual)
build_cnfctl_series <- function(hist_vals, cnfctl_summary, fcst_hor) {
  n_hist <- length(hist_vals)
  list(
    med = c(hist_vals, cnfctl_summary$med[1:fcst_hor]),
    lb = c(hist_vals, cnfctl_summary$lb[1:fcst_hor]),
    ub = c(hist_vals, cnfctl_summary$ub[1:fcst_hor])
  )
}

# RANK series
pi_rank_plot <- build_cnfctl_series(pi_hist, pi_rank_scen, fcst_hor_fig8)
y_rank_plot <- build_cnfctl_series(y_hist, y_rank_scen, fcst_hor_fig8)
i_rank_plot <- build_cnfctl_series(i_hist, i_rank_scen, fcst_hor_fig8)

# HANK series
pi_hank_plot <- build_cnfctl_series(pi_hist, pi_hank_scen, fcst_hor_fig8)
y_hank_plot <- build_cnfctl_series(y_hist, y_hank_scen, fcst_hor_fig8)
i_hank_plot <- build_cnfctl_series(i_hist, i_hank_scen, fcst_hor_fig8)

# B-RANK series
pi_brank_plot <- build_cnfctl_series(pi_hist, pi_brank_scen, fcst_hor_fig8)
y_brank_plot <- build_cnfctl_series(y_hist, y_brank_scen, fcst_hor_fig8)
i_brank_plot <- build_cnfctl_series(i_hist, i_brank_scen, fcst_hor_fig8)

# Empirical series
pi_emp_plot <- build_cnfctl_series(pi_hist, pi_emp_scen, fcst_hor_fig8)
y_emp_plot <- build_cnfctl_series(y_hist, y_emp_scen, fcst_hor_fig8)
i_emp_plot <- build_cnfctl_series(i_hist, i_emp_scen, fcst_hor_fig8)

# Create 3x3 grid of plots
n_pts <- length(plot_dates_fig8)

# Build long-format data frame for faceting
# Row 1: RANK vs HANK
# Row 2: RANK vs B-RANK
# Row 3: Semi-structural only

df_fig8 <- rbind(
  # Row 1: RANK vs HANK - Output
  data.frame(date = plot_dates_fig8, history = y_hist_plot, baseline = y_base_plot,
             model1_med = y_rank_plot$med, model1_lb = y_rank_plot$lb, model1_ub = y_rank_plot$ub,
             model2_med = y_hank_plot$med, model2_lb = y_hank_plot$lb, model2_ub = y_hank_plot$ub,
             row = "RANK vs HANK", variable = "Output"),
  # Row 1: RANK vs HANK - Inflation
  data.frame(date = plot_dates_fig8, history = pi_hist_plot, baseline = pi_base_plot,
             model1_med = pi_rank_plot$med, model1_lb = pi_rank_plot$lb, model1_ub = pi_rank_plot$ub,
             model2_med = pi_hank_plot$med, model2_lb = pi_hank_plot$lb, model2_ub = pi_hank_plot$ub,
             row = "RANK vs HANK", variable = "Inflation"),
  # Row 1: RANK vs HANK - Interest Rate
  data.frame(date = plot_dates_fig8, history = i_hist_plot, baseline = i_base_plot,
             model1_med = i_rank_plot$med, model1_lb = i_rank_plot$lb, model1_ub = i_rank_plot$ub,
             model2_med = i_hank_plot$med, model2_lb = i_hank_plot$lb, model2_ub = i_hank_plot$ub,
             row = "RANK vs HANK", variable = "Interest Rate"),
  
  # Row 2: RANK vs B-RANK - Output
  data.frame(date = plot_dates_fig8, history = y_hist_plot, baseline = y_base_plot,
             model1_med = y_rank_plot$med, model1_lb = y_rank_plot$lb, model1_ub = y_rank_plot$ub,
             model2_med = y_brank_plot$med, model2_lb = y_brank_plot$lb, model2_ub = y_brank_plot$ub,
             row = "RANK vs B-RANK", variable = "Output"),
  # Row 2: RANK vs B-RANK - Inflation
  data.frame(date = plot_dates_fig8, history = pi_hist_plot, baseline = pi_base_plot,
             model1_med = pi_rank_plot$med, model1_lb = pi_rank_plot$lb, model1_ub = pi_rank_plot$ub,
             model2_med = pi_brank_plot$med, model2_lb = pi_brank_plot$lb, model2_ub = pi_brank_plot$ub,
             row = "RANK vs B-RANK", variable = "Inflation"),
  # Row 2: RANK vs B-RANK - Interest Rate
  data.frame(date = plot_dates_fig8, history = i_hist_plot, baseline = i_base_plot,
             model1_med = i_rank_plot$med, model1_lb = i_rank_plot$lb, model1_ub = i_rank_plot$ub,
             model2_med = i_brank_plot$med, model2_lb = i_brank_plot$lb, model2_ub = i_brank_plot$ub,
             row = "RANK vs B-RANK", variable = "Interest Rate"),
  
  # Row 3: Semi-structural - Output
  data.frame(date = plot_dates_fig8, history = y_hist_plot, baseline = y_base_plot,
             model1_med = y_emp_plot$med, model1_lb = y_emp_plot$lb, model1_ub = y_emp_plot$ub,
             model2_med = NA, model2_lb = NA, model2_ub = NA,
             row = "Semi-structural", variable = "Output"),
  # Row 3: Semi-structural - Inflation
  data.frame(date = plot_dates_fig8, history = pi_hist_plot, baseline = pi_base_plot,
             model1_med = pi_emp_plot$med, model1_lb = pi_emp_plot$lb, model1_ub = pi_emp_plot$ub,
             model2_med = NA, model2_lb = NA, model2_ub = NA,
             row = "Semi-structural", variable = "Inflation"),
  # Row 3: Semi-structural - Interest Rate
  data.frame(date = plot_dates_fig8, history = i_hist_plot, baseline = i_base_plot,
             model1_med = i_emp_plot$med, model1_lb = i_emp_plot$lb, model1_ub = i_emp_plot$ub,
             model2_med = NA, model2_lb = NA, model2_ub = NA,
             row = "Semi-structural", variable = "Interest Rate")
)

df_fig8$row <- factor(df_fig8$row, levels = c("RANK vs HANK", "RANK vs B-RANK", "Semi-structural"))
df_fig8$variable <- factor(df_fig8$variable, levels = c("Output", "Inflation", "Interest Rate"))

# Since colors differ by row, we need separate plots per row
library(patchwork)

# Function to create one row of plots
create_row_plot <- function(df_row, row_name, col_m1, col_lm1, col_m2, col_lm2, 
                             show_model2 = TRUE) {
  p <- ggplot(df_row, aes(x = date)) +
    # Model 1 band
    geom_ribbon(aes(ymin = model1_lb, ymax = model1_ub), fill = col_lm1, na.rm = TRUE) +
    # Model 1 median
    geom_line(aes(y = model1_med), color = col_m1, linewidth = 1.2, na.rm = TRUE)
  
  if (show_model2) {
    p <- p +
      # Model 2 band
      geom_ribbon(aes(ymin = model2_lb, ymax = model2_ub), fill = col_lm2, na.rm = TRUE) +
      # Model 2 median
      geom_line(aes(y = model2_med), color = col_m2, linewidth = 1.2, na.rm = TRUE)
  }
  
  p <- p +
    # Grey dashed: VAR baseline forecast
    geom_line(aes(y = baseline), color = col_grey, linetype = "dashed", linewidth = 1.2) +
    # Black solid: history
    geom_line(aes(y = history), color = col_black, linewidth = 1.2, na.rm = TRUE) +
    geom_hline(yintercept = 0, color = "grey70") +
    facet_wrap(~variable, scales = "free_y", ncol = 3) +
    labs(subtitle = row_name, x = "Date", y = "% Deviation") +
...
p_row1 <- create_row_plot(
  df_fig8[df_fig8$row == "RANK vs HANK", ],
  "RANK (beige) vs HANK (red)",
  col_beige, col_lbeige, col_red, col_lred, show_model2 = TRUE
)

# Row 2: RANK (beige) vs B-RANK (blue)
p_row2 <- create_row_plot(
  df_fig8[df_fig8$row == "RANK vs B-RANK", ],
  "RANK (beige) vs B-RANK (blue)",
  col_beige, col_lbeige, col_blue, col_lblue, show_model2 = TRUE
)

# Row 3: Semi-structural (orange)
p_row3 <- create_row_plot(
  df_fig8[df_fig8$row == "Semi-structural", ],
  "Semi-structural (orange)",
  col_orange, col_lorange, NA, NA, show_model2 = FALSE
)

# Combine rows
(p_row1 / p_row2 / p_row3) +
  plot_annotation(
    title = "Figure 8: Counterfactual Projections Under Optimal Policy",
    subtitle = paste0("Post-", date_sample[scenario_date], " scenario. ",
                       "Black: history. Grey dashed: VAR baseline. ",
                       "Colored: posterior median with 68% bands.")
  )
```
